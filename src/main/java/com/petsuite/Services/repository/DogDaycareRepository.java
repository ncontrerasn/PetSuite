...
public interface DogDaycareRepository extends JpaRepository<DogDaycare, String>{
    ...
    @Query(value = "SELECT user FROM dog_daycare where dog_daycare_score BETWEEN ?2 AND ?1", nativeQuery = true)
    List<String> searchByScore(Float scoreUp, Float scoreDown);
